...
import { Observable, debounceTime, distinctUntilChanged, filter, map, of } from 'rxjs';
  ...
  template: `
    <div class="home-page">
      <mat-form-field>
        ...
        <input matInput type="text" placeholder="Pesquisar" [(ngModel)]="queryField" (input)="onSearchChange($event.target)">
      </mat-form-field>
      <div class="product-items">
        <app-product-list panelTitle="Produtos" [productList]="filtredProductItems()" />
      </div>
    </div>
  `,
...
export class HomeComponent {
  ...
  productItems = signal<ProductItem[]>([
    {
       id:1,
  ...
  filtredProductItems = signal<ProductItem[]>(this.productItems());

  constructor() {}

  onSearchChange(target: any): void {
    this.filtredProductItems.set(this.filterItems(target.value));
  }

  private filterItems(query: string): ProductItem[] {
